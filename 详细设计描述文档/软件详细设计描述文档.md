...
|ScheduleMapper.insertOneMovie(MovieForm addMovieForm)|插入单一的持久化对象|
|ScheduleMapper.selectMovieById(int id)|根据id返回单一持久化对象|
|ScheduleMapper.selectMovieByIdAndUserId(int id, int userId)|根据id和userId查找电影|
|ScheduleMapper.selectAllMovie()|返回所有电影对象|
|ScheduleMapper.selectOtherMoviesExcludeOff()|展示不包括已经下架的所有的电影对象|
|ScheduleMapper.selectMovieByKeyword(String keyword)|根据关键字搜索电影|
|ScheduleMapper.updateMovieStatusBatch(List< Integer> movieIdList)|批量更新电影状态|
|ScheduleMapper.updateMovie(MovieForm updateMovieForm)|修改电影|
