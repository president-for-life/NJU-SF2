...
public class TicketServiceImpl implements TicketService, TicketServiceForBl {
    ...
    @Override
    @Transactional
    public ResponseVO completeTicket(List<Integer> ticketIds, int couponId) {
        //////////////////////////////////////////////////////
        ////////////////////控制台测试信息////////////////////
        ...
        if (!this.areExpired(ticketIds)) { // 电影票未超时
    ...
    @Override
    @Transactional
    public ResponseVO completeByVIPCard(List<Integer> ticketIds, int couponId) {
        try {
            //////////////////////////////////////////////////////
            ////////////////////控制台测试信息////////////////////
            ...
            Coupon coupon = couponService.getCouponById(couponId); // 使用的优惠券
            boolean noCoupon = (coupon == null); // 判断是否存在 couponId 对应的优惠券
            double pay = scheduleItem.getFare() * ticketIds.size()
                    - (noCoupon ? 0 : coupon.getDiscountAmount());

            // 判断电影票是否超时
            boolean success = !this.areExpired(ticketIds);
    ...
    public ResponseVO cancelTicket(List<Integer> ticketIds) {
        try {
            ticketIds.forEach(ticketId -> {
                ...
                if (ticket.getState() == 0) { // 只更改状态为“未完成”的座位
                    ticketMapper.updateTicketState(ticketId, 2); // 更改状态为“已失效”
                }
            });
